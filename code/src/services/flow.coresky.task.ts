...
import { CoreskyAutomation } from '../services/flow/coresky.flow';
...
class AutoworkFlowsTaskExecutor {
    ...
    constructor(inputFile: string, outputFile: string,codesFile: string,codesKeysFile: string,) {
      ...
      this.proxies = []; // 所有代理信息
      this.keysAndCodes = []; // 私钥与代理映射表
    }
  
    // 初始化：读取私钥
    async initialize(): Promise<void> {
        // // 读取私钥文件
        // var input = await importFromFile(this.inputFile);
        // // 输出目录可能不存在
        ...
        // 读取邀请码候选列表
        // 邀请码全局保存，每行一个邀请码
        globalReferralCode.CORESKY_REFERRAL_CODE = await importFromFile(this.codesFile);
        // 私钥与邀请码绑定表
    ...
    async runSingleTask(selected:[string,number],inviteCode:string): Promise<boolean> {
      // 工作流客户端
      const cliet = new CoreskyAutomation("okx");    
      // 抽取私钥
      ...
      var result = 'fail';
      var index = 0;
      while(index<2) {   // 这个交互不需要重试多次
        result = await cliet.run(privateKey,proxyStr, inviteCode); // 结果如是ok，则不需要重试
        if(result == "ok"){
          index = 9;
          result = "ok";
        } else {
    ...
    async runAllTasks(): Promise<void> {
      ...
      while (hasMoreTasks) {
        ...
        if (binding) {
          ...
          // 从邀请码表中随机获取
          inviteCode = getRandomElement(globalReferralCode.CORESKY_REFERRAL_CODE);
      ...
      await delay(10*60*1000); // 模拟任务间隔
    }
  }
  
  // 主函数
  export async function runCoreskyAutoFlow() {

    // 读取文件
    const inputFile = path.join(__dirname, '../','data','task', 'coresky_keys.txt');
    const outputFile = path.join(__dirname,'../','data', 'task', `coresky.${getCurrentDateString()}.txt`);
    const codesFile = path.join(__dirname,'../','data', 'task', `coresky_code.txt`);
    const codesKeysFile = path.join(__dirname,'../','data', 'task', `coresky_code_keys.txt`);
