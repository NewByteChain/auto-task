...
import { MahojinAutomation } from './flow/mahojin.ai.flow';
...
class AutoworkFlowsTaskExecutor {
    ...
    async runSingleTask(selected:[string,number]): Promise<boolean> {
      // 工作流客户端
      const cliet = new MahojinAutomation("okx");    
      // 抽取私钥
      ...
      var index = 0;
      while(index<2){
        result = await cliet.run(privateKey,proxyStr); // 结果如是ok，则不需要重试
        if(result == "ok"){
          index = 9;
          result = "ok";
        } else {
  ...
  // 主函数
  export async function runMahojinAutoFlow() {

    // 读取文件
    const inputFile = path.join(__dirname, '../','data','task', 'mahojin_keys.txt');
    const outputFile = path.join(__dirname,'../','data', 'task', `mahojin.${getCurrentDateString()}.txt`);
