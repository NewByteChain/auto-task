...
import { Browser, Page, Target ,ElementHandle } from 'puppeteer';
...
import { connectWalletOkx,importWalletOkx,connectWalletOkxConfirm,connectWalletOkxConfirm2 } from "../wallet.plugins/okx.wallet.plugins"; // 钱包插件
 ...
 * Klokapp 钱包自动化
 */
export class KlokappAutomation {
  private browser: Browser | null = null;
  private page: Page | null = null;
  private wallet: WalletConfig;
  private walletSetp:number | null = null;
                ...
                if (p) {
                    ...
                    try{
                      ...
                      if(this.walletSetp == 1){
                        ...
                        await connectWalletOkxConfirm(p);
  ...
  async initializeApp(randomDirName:string,proxyInfo:ProxyConfig|null): Promise<void> {
    ...
    // 等待10s
    await delay(10000 + Math.ceil(Math.random() * 1000)); // 随机休眠
    // 等待10秒，直接关闭浏览器
    await browser.close();
   ...
   * 登录网站
   */
  async signupWebSite(url:string): Promise<void> {
    if (!this.page || !this.browser) throw new Error('Browser or Page not initialized');
    ...
    try{ 
      ...
      await delay(Math.ceil(Math.random() * 9000) + 1000); // 随机休眠
   ...
   */
  @timeout(600000)
  async performSwap(url:string,questions:string[]): Promise<void> {
    ...
    const link = await this.page.evaluate(() => {
      const input = document.querySelector('#puppeteer-clipboard-input') as HTMLInputElement;
      return input?.value;
    });
    ...
    if(link){
        // 当获取内容时候
        const referral_code = getUrlParameter(link,"referral_code");
        if(referral_code) {
            // 需要先检测全局变量，邀请码列表中是否存在本轮邀请码（去重写入）
            const code = globalReferralCode.KLOKAPP_REFERRAL_CODE.find(c => c === referral_code);
            console.log(`[Perform Swap] 检索全局变量中是否存在本次任务得邀请码:${code}`);
            if(!code){
              // 如果没找到，该账号是第一次出现，记录到全局变量中，并写入文件中
              globalReferralCode.KLOKAPP_REFERRAL_CODE.push(referral_code);
              // 追加写入文件
              await appendResultToFile(path.join(__dirname,'../../','data', 'task', `klokapp_code.txt`), referral_code);
    ...
    for(var i in questions){
      ...
      await delay(Math.ceil(Math.random() * 200) + 200); // 随机休眠，2.6s-3.2s
      
      // 填充问题
      await this.page.waitForSelector(inputSelector,{ visible: true });
      await this.page.type(inputSelector,questions[i]);
      await delay(Math.ceil(Math.random() * 2000) + 1000); // 随机休眠，2.6s-3.2s
      ...
      // 问题答案打印时间普遍在20s，页面停留
      await delay(Math.ceil(Math.random() * 25000) + 5000); // 随机休眠
   ...
   * @returns 
   */
  async run(privateKey:string,proxyStr:string,qs:string[],inviteCode:string): Promise<string> {
    try {
