 ...
 * 连接OKX钱包，带密码输入
 */
async function connectWalletOkxByPassword(page: Page, OKX_WALLET_PASSWORD:string):Promise<void> {
 ...
 * @returns 
 */
async function confirmWalletOkx(page: Page):Promise<void> {
    try{
 ...
 * @returns 
 */
async function connectWalletOkxAndConfirm(page: Page):Promise<void> {
    try{
...
export { 
    ...
    importWalletOkx , 
    // 连接OKX钱包，带密码输入
    connectWalletOkxByPassword,
    // 操作钱包连接确认操作
    confirmWalletOkx,
    // 连接钱包 + 确认连接合并交互
    connectWalletOkxAndConfirm,
};